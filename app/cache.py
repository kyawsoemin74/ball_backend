...
def make_cache_key(*parts: Any) -> str:
    return ":".join([settings.REDIS_CACHE_PREFIX] + [str(part) for part in parts])


def _serialize(value: Any) -> str:
    return json.dumps(jsonable_encoder(value), separators=(",", ":"), sort_keys=True)
...
def cache_set_json_sync(key: str, value: Any, ttl: int) -> None:
    sync_redis.set(key, _serialize(value), ex=ttl)
...
async def cache_set_json(key: str, value: Any, ttl: int) -> None:
    await async_redis.set(key, _serialize(value), ex=ttl)
