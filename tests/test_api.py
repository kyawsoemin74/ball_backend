...
@pytest.mark.parametrize("status", ["NS", "1H", "HT", "2H", "ET", "LIVE", "FT", "AET", "PEN"])
def test_has_events_uses_event_service_fallback_for_all_statuses(monkeypatch, status):
    from app.api.matches import _build_match_availability_flags
    ...
    async def fake_get_cached_match_events(db, match_id):
        called["events"] += 1
        return [{"id": None, "match_id": match_id, "team_name": "Fallback Team"}]
    ...
    match = SimpleNamespace(match_id=105, home_team_id=1, away_team_id=2, status=status)
    flags = asyncio.run(_build_match_availability_flags(match, FakeDB()))
...
@pytest.mark.parametrize("status", ["NS", "1H", "HT", "2H", "ET", "LIVE", "FT", "AET", "PEN"])
def test_has_events_returns_false_when_event_service_fallback_is_empty_for_all_statuses(monkeypatch, status):
    from app.api.matches import _build_match_availability_flags
    ...
    match = SimpleNamespace(match_id=106, home_team_id=1, away_team_id=2, status=status)
    flags = asyncio.run(_build_match_availability_flags(match, FakeDB()))
